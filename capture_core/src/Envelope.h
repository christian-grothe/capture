...
public:
  ...
  enum EnvState { Attack, Release, Hold };

  EnvState state = EnvState::Attack;

  void reset() { level = SILENCE; }

  void attack() { state = EnvState::Attack; }

  void release() { state = EnvState::Release; }
  ...
  void init(float sampleRate, float initAttRel,
            std::function<void()> onEndedCb) {
