...
template <typename T> inline static T lerp(T v0, T v1, T t) {
  return (1 - t) * v0 + t * v1;
}

template <typename Float>
inline Float cubicHermiteSpline(const Float *buffer, const Float readHead,
  ...
  const auto c2 = v0 - static_cast<Float>(2.5) * v1 +
                  static_cast<Float>(2.) * v2 - static_cast<Float>(.5) * v3;
  const auto c3 =
      static_cast<Float>(1.5) * (v1 - v2) + static_cast<Float>(.5) * (v3 - v0);
